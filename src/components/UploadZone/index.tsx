...
const loadFile = (file: FileWithPreview): string => {
  return URL.createObjectURL(file);
};

const copyFileToTemp = async (filePath: string): Promise<string> => {
  const tempPath = await ipcRenderer.invoke('copy-file', filePath);
  return tempPath;
};
...
const UploadZone: React.FC = () => {
    ...
    if (e.target.files) {
      ...
      const newFiles: FileWithPreview[] = await Promise.all(
        fileArray.map(async (file) => {
          const tempPath = await copyFileToTemp((file as any).path);
          return {
            ...file,
            preview: loadFile(file as FileWithPreview),
            tempPath,
          } as FileWithPreview;
        })
      );
  ...
  const dragEnter = (e: DragEvent<HTMLDivElement>, index: number) => {
    e.preventDefault();
  ...
  return (
    <div className="bg-white p-7 rounded w-9/12 mx-auto">
      <div className="relative flex flex-col p-4 text-gray-400 border border-gray-200 rounded">
        ...
        >
          <input
            ...
            onDrop={(e) => {
              e.preventDefault();
              dndRef.current?.classList.remove('border-blue-400', 'ring-4', 'ring-inset');
              addFiles(e as any as ChangeEvent<HTMLInputElement>); // TSX conversion workaround
            }}
          />
        ...
        {files.length > 0 && (
          <div className="grid grid-cols-2 gap-4 mt-4 md:grid-cols-6" onDrop={drop} onDragOver={(e) => e.preventDefault()}>
            {files.map((file, index) => (
              <div
                ...
                data-index={index}
              >
                <button className="absolute top-0 right-0 z-50 p-1 bg-white rounded-bl focus:outline-none" type="button" onClick={() => removeFile(index)}>
                ...
                {(file.type.includes('application/') || file.type === '') && (
                  <svg className="absolute w-12 h-12 text-gray-400 transform top-1/2 -translate-y-2/3"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                      d="M7 21h10a2 2 002-2V9.414a1 1 00-.293-.707l-5.414-5.414A1 1 0012.586 3H7a2 2 002 2v14a2 2 002 2z" />
                  </svg>
                )}
